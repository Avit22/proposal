...
class Input extends CI_Controller {
	...
	public function index() {
		...
		$this->load->view('kaprodi_ta/input_proposal',$data);
		
	}

	public function index2() {

				$this->load->view('kaprodi_ta/cek_proposal');

	}

	function hapus($id) {
		$this->Input_model->hapus($id);
		redirect('kaprodi_ta/terkirim');
	}
...
public function update_review($id) {
	...
	$this->form_validation->set_rules('kode', 'Kode Proposal', 'required');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->load->view('kaprodi_ta/cek_proposal2',$data);
		}
