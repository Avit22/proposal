...
class Insert_rab extends CI_Controller{
	...
	function __Construct(){
		...
		if(!$this->session->userdata('logged_in')) {
   		...
   		elseif($this->session->userdata('tingkatan') != "kaprodi_tm" && $this->session->userdata('tingkatan') != "kaprodi_tm"){
   		redirect('login', 'refresh');
	   	}
	...
	function update($id_proposal,$id_rab){
		...
		$this->load->view('kaprodi_tm/update_rab',$data);
	}
	...
	function update1($id_proposal,$id_rab){
		...
		$this->load->view('kaprodi_tm/update_rab1',$data);
	}
	...
	if ($this->form_validation->run() == FALSE) {
		...
		$tgl = date("Y-m-d");
			...
			if($this->Input_model->tambah_rab($data));
				redirect('kaprodi_tm/insert_rab/index/'.$id_proposal);	
	}
}
...
public function add_rab1($id_proposal){
	...
	if ($this->form_validation->run() == FALSE) {
		...
		$tgl = date("Y-m-d");
			...
			if($this->Input_model->tambah_rab($data));
				redirect('kaprodi_tm/detail_rab/detail/'.$id_proposal);	
	}
}
...
public function delete_rab1($id_proposal,$id_rab){
	...
	$this->form_validation->set_rules('catatan', 'catatan', 'required');
		...
		redirect('kaprodi_tm/notifikasi_rab');	
}
}
