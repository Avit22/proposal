...
class Insert_rab extends CI_Controller{
	...
	function index($id_proposal){
		...
		$this->load->view('kajur_tm/insert_rab',$data);
	}
	...
	function update($id_proposal,$id_rab){
		...
		$this->load->view('kajur_tm/update_rab',$data);
	}
	...
	function update1($id_proposal,$id_rab){
		...
		$this->load->view('kajur_tm/update_rab1',$data);
	}
	...
	if ($this->form_validation->run() == FALSE) {
		...
		$tgl = date("Y-m-d");
			...
			if($this->Input_model->tambah_rab($data));
				redirect('kajur_tm/insert_rab/index/'.$id_proposal);	
	}
}
...
public function add_rab1($id_proposal){
	...
	if ($this->form_validation->run() == FALSE) {
		...
		$tgl = date("Y-m-d");
			...
			if($this->Input_model->tambah_rab($data));
				redirect('kajur_tm/detail_rab/detail/'.$id_proposal);	
	}
}
...
public function delete_rab1($id_proposal,$id_rab){
	...
	$this->form_validation->set_rules('catatan', 'catatan', 'required');
		...
		redirect('kajur_tm/notifikasi_rab');	
}
}
