...
class Input extends CI_Controller {
	...
	public function index() {
		...
		$this->load->view('kaprodi_te/input_proposal',$data);
		
	}

	public function index2() {

				$this->load->view('kaprodi_te/cek_proposal');

	}

	function hapus($id) {
		$this->Input_model->hapus($id);
		redirect('kaprodi_te/terkirim');
	}
...
public function update_review($id) {
	...
	$this->form_validation->set_rules('kode', 'Kode Proposal', 'required');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->load->view('kaprodi_te/cek_proposal2',$data);
		}
