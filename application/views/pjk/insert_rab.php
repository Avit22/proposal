...
<html lang="en"> 
  ...
  <br />
  <?php echo form_open('pjk/insert_rab/add_rab/'.$id_proposal,array('id' => 'tambah','name' => 'tambah', 'class' => 'form-horizontal')); ?>
 ...
 <table class="table table-striped" id="myTable">
    ...
    if(isset($rab)){
      foreach ($rab as $row){
      echo "<tr>";
      echo "<td>".$row->barang."</td><td>".rupiah3($row->harga)."</td><td>".$row->jumlah."</td><td>".rupiah3($row->total)."</td>"."<td><a href='".base_url('pjk/insert_rab/update/').'/'.$id_proposal.'/'.$row->id."'>UPDATE</a></td><td><a href='".base_url('pjk/insert_rab/delete_rab/').'/'.$id_proposal.'/'.$row->id."'>DELETE</a></td>";
      echo "</tr>";
    }
    }
    if(isset($totalrab)){
